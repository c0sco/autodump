...
#
## autodump was written for FreeBSD, and tested on Ubuntu 13.10 and CentOS 6.4.
## Let me know if you have success elsewhere.
#
## System requirements:
...
# Any device in here with a dump flag of '1' will be dumped using its mount point without slashes as the file name.
...
# For Ubuntu you might want to set this to /var/lib/dumpdates. See `man dump`.
DUMPDATES=

# Directory with a list of shell scripts that will be run when a backup is completed.
# Scripts are called with the following args: $BACKUP_DEST $current_set_dir $level
# e.g. /usr/local/etc/autodump.d/encsend.sh /nfs/system_backup/dump set0 0
TRIGGERS=/usr/local/etc/autodump.d
...
find_bin()
{
    found=`which $1`
    if [[ "$found" == "" ]]; then
        echoerr "Can't find $1 in your \$PATH. Exiting."
        kill -s TERM $TOP_PID
    fi
...
find_flags()
{
	echo $(eval echo $(echo $`echo ${1}_${OS}`))
}
...
get_current_set ()
{
	echo `$FINDBIN $BACKUP_DEST -type f | $XARGSBIN $STATBIN $STATFL | $SORTBIN -nr | $CUTBIN -d : -f2- | $HEADBIN -n1 | $SEDBIN 's/.*set\([0-9]\).*/\1/'`
}

# Find the path to the oldest level 0 dump (i.e. the oldest "complete" thing we got)
get_oldest_zero ()
{
	THEPATH=`$FINDBIN $BACKUP_DEST -type f | $XARGSBIN $STATBIN $STATFL | $SORTBIN -nr | $CUTBIN -d : -f2- | $GREPBIN "0.dump.bz2" | $TAILBIN -n1`
...
get_last_lvl ()
{
	echo `$DUMPBIN -W | $GREPBIN -v "^Last" | $TAILBIN -n 1 | $SEDBIN -e 's/.*Level \([0-9]\).*/\1/' -e 's/.*never$//'`
}

# Check permissions on things we need to use
check_perms ()
{
	if [[ -e "${DUMPDATES:-/etc/dumpdates}" && ! -r "${DUMPDATES:-/etc/dumpdates}" ]]; then
		echo "Can't read ${DUMPDATES:-/etc/dumpdates}! Exiting."
		exit
	fi

	if [[ ! -r "$BACKUP_DEST" || ! -x "$BACKUP_DEST" ]]; then
		echo "Can't read or traverse $BACKUP_DEST! Exiting."
		exit
	fi

	if [[ -e "${DUMPDATES:-/etc/dumpdates}" && ! -w "${DUMPDATES:-/etc/dumpdates}" ]]; then
...
DEVICES=`$CATBIN $FSTAB_FILE | $AWKBIN '{print $1 ":" $2 ":" $5}' | $GREPBIN -v ":0" | $GREPBIN -v "^#" | $GREPBIN -v "::" | $CUTBIN -d : -f1-2 | $SEDBIN -e 's/\/$/root/' -e 's/:\//:/' | $XARGSBIN echo`
...
echo "@@@ Beginning dump to $DUMP_PATH @@@"
...
for CURDEVLABEL in $DEVICES
 do
	...
	# Do the dump.
	$DUMPBIN -$NEXTDUMP -h $NODUMPCARE $DUMPFL - $CUR_DEV | $BZ2BIN > $DUMP_PATH/$CUR_FILE$DUMP_FILE_SUFFIX
	# Calculate the md5.
...
echo "@@@ Writing $LABEL_FILE @@@"
DUMP_LISTING=`$LSBIN $LSFL $DUMP_PATH`
...
# Do we have triggers to run?
if [ -d "$TRIGGERS" ]; then
	echo "@@@ Running triggers in $TRIGGERS @@@"
	for t in $(eval echo ${TRIGGERS}/*.sh)
		do echo "Running trigger `$BASENAMEBIN $t`"
		$t $BACKUP_DEST set${SET} $NEXTDUMP
	done
fi
